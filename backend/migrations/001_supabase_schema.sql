-- =============================================================================
-- GCC Bond Trading Platform — Supabase-Compatible Schema
-- Differences from 001_initial_schema.sql:
--   1. "auth" schema renamed to "app_auth" (Supabase owns the "auth" schema)
--   2. TimescaleDB made optional (not available on Supabase free tier)
-- Run order: this file first, then 002_seed_data.sql
-- =============================================================================

-- Extensions (pgcrypto and pg_trgm are available on all Supabase plans)
CREATE EXTENSION IF NOT EXISTS "pgcrypto";
CREATE EXTENSION IF NOT EXISTS "pg_trgm";

-- TimescaleDB is only available on Supabase paid plans.
-- Uncomment the line below if you have it enabled under
-- Supabase Dashboard → Database → Extensions → timescaledb
-- CREATE EXTENSION IF NOT EXISTS "timescaledb";

-- =============================================================================
-- SCHEMA: app_auth  (renamed from "auth" to avoid Supabase conflict)
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS idx_users_email_hash ON app_auth.users (email_hash);
CREATE INDEX IF NOT EXISTS idx_users_status     ON app_auth.users (status);
CREATE INDEX IF NOT EXISTS idx_users_role       ON app_auth.users (role);
CREATE INDEX IF NOT EXISTS idx_users_name_trgm  ON app_auth.users USING GIN (name_normalized gin_trgm_ops);
...
CREATE TABLE IF NOT EXISTS kyc.submissions (
    ...
    reviewer_id         UUID        REFERENCES app_auth.users(id),
    review_notes        TEXT,
...
CREATE TABLE IF NOT EXISTS wallet.wallets (
    id                  UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id             UUID         NOT NULL UNIQUE REFERENCES app_auth.users(id),
...
CREATE TABLE IF NOT EXISTS wallet.withdrawal_requests (
    ...
    maker_id           UUID         REFERENCES app_auth.users(id),
    checker_id         UUID         REFERENCES app_auth.users(id),
    checker_notes      TEXT,
...
-- Price history as a regular table (TimescaleDB hypertable skipped for Supabase free tier)
-- If you enable TimescaleDB, add: SELECT create_hypertable('bonds.price_history', 'recorded_at', if_not_exists => TRUE);
...
CREATE TABLE IF NOT EXISTS trading.trades (
    ...
    buyer_id        UUID          NOT NULL REFERENCES app_auth.users(id),
    seller_id       UUID          NOT NULL REFERENCES app_auth.users(id),
    bond_id         UUID          NOT NULL REFERENCES bonds.listings(id),
    quantity        DECIMAL(18,4) NOT NULL CHECK (quantity > 0),
...
-- =============================================================================
-- Hangfire schema (used by Settlement service job scheduler)
-- =============================================================================

CREATE SCHEMA IF NOT EXISTS hangfire;

-- =============================================================================
-- updated_at auto-trigger (applies to all tables with that column)
-- =============================================================================
...
BEGIN
    FOR t IN
        ...
        WHERE column_name = 'updated_at'
          AND table_schema IN ('app_auth','kyc','wallet','bonds','trading','settlement','portfolio','aml')
    LOOP
        EXECUTE format(
